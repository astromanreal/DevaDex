...
import type { AsuraCategoryData, NotableAsura } from '@/data/asura-category-data';
import Image from 'next/image';
import Link from 'next/link';
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { AlertTriangle, ShieldAlert, BookOpen, Users, ExternalLink, ArrowRight, Skull, Flame } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from '@/components/ui/accordion';

interface AsuraCategoryPageDisplayProps {
  categoryData: AsuraCategoryData;
}

const AsuraDetailCard: React.FC<{ asura: NotableAsura }> = ({ asura }) => (
      ...
      <div className="flex flex-col sm:flex-row items-start gap-4">
        <div className="relative w-full sm:w-1/3 aspect-[4/3] rounded-md overflow-hidden shrink-0 shadow-md">
          <Image
            ...
            layout="fill"
            objectFit="cover"
            data-ai-hint={asura.imageAiHint}
          />
        </div>
        <div className="flex-grow">
          <CardTitle className="text-xl text-primary group-hover:text-accent transition-colors">{asura.name}</CardTitle>
          <CardDescription className="text-sm text-muted-foreground">{asura.type} - From {asura.epic || 'Various Texts'}</CardDescription>
          {asura.kingdom && <p className="text-xs text-muted-foreground">Kingdom: {asura.kingdom}</p>}
          {asura.yuga && <p className="text-xs text-muted-foreground">Period: {asura.yuga}</p>}
        </div>
      </div>
    </CardHeader>
    <CardContent className="p-4 pt-0 text-sm space-y-3 flex-grow">
      <div>
        ...
        <div className="flex flex-wrap gap-1">
            {asura.traits.map((trait, index) => <Badge key={index} variant="outline" className="text-xs">{trait}</Badge>)}
        </div>
      </div>
      <div>
        ...
        <ul className="list-disc list-inside pl-2 space-y-0.5 text-xs">
          {asura.notable_deeds.map((deed, index) => <li key={index} className="truncate" title={deed}>{deed}</li>)}
        </ul>
      </div>
       <p><strong>Defeated By:</strong> {asura.defeated_by}</p>
      <p className="line-clamp-2"><strong>Symbolism:</strong> {asura.symbolism}</p>
    </CardContent>
    <CardFooter className="p-4 pt-0">
      <Link href={`/characters/${asura.id}`} passHref>
...
export function AsuraCategoryPageDisplay({ categoryData }: AsuraCategoryPageDisplayProps) {
  if (!categoryData) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-lg text-muted-foreground mb-6">
          The data for the Asura category could not be loaded.
        </p>
  ...
  return (
    <Card className="shadow-xl overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row gap-6 items-start">
          {categoryData.imageUrl && (
            <div className="w-full md:w-1/3 relative aspect-video rounded-lg overflow-hidden shadow-lg border-4 border-card">
              <Image
                ...
                layout="fill"
                objectFit="cover"
                data-ai-hint={categoryData.imageAiHint || 'asura beings powerful'}
          ...
          <div className={`w-full ${categoryData.imageUrl ? 'md:w-2/3' : 'md:w-full'}`}>
            <div className="flex items-center mb-2">
              <Flame className="h-10 w-10 mr-3 text-primary" /> {/* Using Flame for Asura */}
              <CardTitle className="text-4xl font-bold text-primary">{categoryData.category}</CardTitle>
            </div>
            <CardDescription className="text-lg text-primary/80 mb-3">Explore notable Asuric figures from Sanatan Dharma.</CardDescription>
      ...
      <CardContent className="p-6 space-y-6">
         <Accordion type="multiple" defaultValue={['notable_asuras', 'symbolic_meaning']} className="w-full">
             <AccordionItem value="notable_asuras">
                <AccordionTrigger className="text-2xl font-semibold text-primary hover:text-accent">
                    <Users className="mr-2" /> Notable Asuras
                </AccordionTrigger>
                <AccordionContent className="pt-4">
                    {categoryData.notable_asuras.length > 0 ? (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        {categoryData.notable_asuras.map((asura) => (
                        <AsuraDetailCard key={asura.id} asura={asura} />
                        ))}
                    </div>
                    ) : (
                    <div className="text-center py-8">
                        <AlertTriangle className="mx-auto h-12 w-12 text-muted-foreground mb-3" />
                        <p className="text-xl text-muted-foreground">No Asuras found in this category data.</p>
                    </div>
                    )}
                </AccordionContent>
            </AccordionItem>
            ...
            <AccordionItem value="asura_subtypes">
                <AccordionTrigger className="text-2xl font-semibold text-primary hover:text-accent">
                    <Users className="mr-2" /> Asura Subtypes
                </AccordionTrigger>
            ...
            <AccordionItem value="scriptures">
                ...
                <AccordionContent className="pt-4 text-foreground">
                    <ul className="list-disc list-inside">
                        {categoryData.scriptures_mentioning_asuras.map(scripture => <li key={scripture}>{scripture}</li>)}
                    </ul>
                </AccordionContent>
            </AccordionItem>
        </Accordion>
